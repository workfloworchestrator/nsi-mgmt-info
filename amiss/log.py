...
class DatabaseLogHandler(Handler):
    def emit(self, record: LogRecord) -> None:
        """Filter structlog generated messages and store circuit related ones in the database."""
        with Session.begin() as session:
            # when msg attribute exists and is a dict, then the message is generated by structlog
            if hasattr(record, "msg") and isinstance(record.msg, dict):
                # check for anything that can map this message to a know circuit
                if "circuitId" in record.msg:
                    circuitId = record.msg["circuitId"]
                elif "connectionId" in record.msg and record.msg["connectionId"] != "None":
                    circuitId = (
                        session.query(Circuit.id)  # type: ignore
                        .filter(Circuit.connectionId == UUID(record.msg["connectionId"]))
                        .scalar()
                    )
                elif "globalReservationId" in record.msg:
                    circuitId = (
                        session.query(Circuit.id)  # type: ignore
                        .filter(Circuit.globalReservationId == UUID(record.msg["globalReservationId"]))
                        .scalar()
                    )
                elif "correlationId" in record.msg:
                    circuitId = (
                        session.query(Circuit.id)  # type: ignore
                        .filter(Circuit.correlationId == UUID(record.msg["correlationId"]))
                        .scalar()
                    )
                else:
                    circuitId = -1

                # Store the message only if it maps to a real circuit. circuitId is None when
                # a connectionId/globalReservationId/correlationId was given but no circuit matches
                # (e.g. during seeding, or a child segment connectionId); such messages are not stored.
                if circuitId is not None and circuitId >= 0:
                    session.add(
                        Log(
