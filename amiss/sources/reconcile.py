...
def _wfo_sdp_pair(sub: SdpSub) -> frozenset[str] | None:
    ids = {nid for member in sub.stps if (nid := normalize_stp_id(member.stp_id))}
    return frozenset(ids) if len(ids) == 2 else None


def _dds_sdp_pair(dds: DdsSdp) -> frozenset[str] | None:
    ids = {nid for raw in (dds.stp_a_id, dds.stp_z_id) if (nid := normalize_stp_id(raw))}
    return frozenset(ids) if len(ids) == 2 else None
...
def reconcile_sdps(wfo: list[SdpSub] | None, dds: list[DdsSdp] | None) -> SdpReconciliation:
    ...
    wfo_by_pair = {pair: s for s in wfo if (pair := _wfo_sdp_pair(s))}
    dds_by_pair = {pair: d for d in dds if (pair := _dds_sdp_pair(d))}
